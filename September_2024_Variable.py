...
def September_2024_Variable(data):
    data_2024_complete = data.groupby(["Category"]).sum(list).reset_index()
    data_2024_Variable_Cost_Data = data[(data["Category"] == "Variable Costs")]
    L1_Grouped = (
        data_2024_Variable_Cost_Data.groupby(["Category"]).sum(list).reset_index()
    )
    L2_Grouped = (
        data_2024_Variable_Cost_Data.groupby(["Category", "Order"])
        .sum(list)
        .reset_index()
    )
    L3_Grouped = (
        data_2024_Variable_Cost_Data.groupby(["Category", "Order", "L3"])
        .sum(list)
        .reset_index()
    )
    L4_Grouped = (
        data_2024_Variable_Cost_Data.groupby(["Category", "Order", "L3", "L4"])
        .sum(list)
        .reset_index()
    )
    L5_Grouped = (
        data_2024_Variable_Cost_Data.groupby(
    ...
    data_Budget = float(data_2024_complete["1st Comparison"].sum())
    data_spent = float(data_2024_complete["Main"].sum())
    data_Delta = float(data_2024_complete["Var Comp&Main"].sum())

...
def October_2025_Fixed(data):
    ...
    with col4:
        st.metric(
            "October Fixed Cost Budget",
            value=f"£{data_Fixed_Budget:,.2f}",
            border=True,
            help="This is the total amount budgeted to be spent for Fixed Costs in October.",
        )
    with col5:
        st.metric(
            "October Fixed Cost Spent",
            value=f"£{data_Fixed_Spent:,.2f}",
            border=True,
            help="This is the total amount spent for Fixed Cost inOctober.",
        )
    with col6:
        st.metric(
            "October Fixed Cost Delta",
            value=f"£{data_Fixed_Delta:,.2f}",
            border=True,
            delta=f"{data_Fixed_Delta_Percent:,.2f}%",
            help="This is the difference in the budgeted amount and Spent amount for Fixed Costs in October.",
        )

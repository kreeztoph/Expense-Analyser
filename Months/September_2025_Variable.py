...
def September_2025_Variable(data):
    data_2025_complete = data.groupby(["Category"]).sum(list).reset_index()
    data_2025_Variable_Cost_Data = data[(data["Category"] == "Variable Costs")]
    L1_Grouped = (
        data_2025_Variable_Cost_Data.groupby(["Category"]).sum(list).reset_index()
    )
    L2_Grouped = (
        data_2025_Variable_Cost_Data.groupby(["Category", "Order"])
        .sum(list)
        .reset_index()
    )
    L3_Grouped = (
        data_2025_Variable_Cost_Data.groupby(["Category", "Order", "L3"])
        .sum(list)
        .reset_index()
    )
    L4_Grouped = (
        data_2025_Variable_Cost_Data.groupby(["Category", "Order", "L3", "L4"])
        .sum(list)
        .reset_index()
    )
    L5_Grouped = (
        data_2025_Variable_Cost_Data.groupby(
    ...
    data_Budget = float(data_2025_complete["1st Comparison"].sum())
    data_spent = float(data_2025_complete["Main"].sum())
    data_Delta = float(data_2025_complete["Var Comp&Main"].sum())
    data_delta_percent = (((data_Budget) - (data_spent)) / (data_Budget)) * 100
    data_Variable_Budget = float(df_parsed_1["Budget"].iloc[0])
    data_Variable_Spent = float(df_parsed_1["Main"].iloc[0])
    data_Variable_Delta = float(df_parsed_1["Delta"].iloc[0])
    data_Variable_Delta_Percent = (
        ((data_Variable_Budget) - (data_Variable_Spent)) / (data_Variable_Budget)
    ) * 100
    ...
    with col1:
        st.metric(
            ...
            border=True,
            help="This is the total budget for September.",
        )
    with col2:
        st.metric(
            "September Spent",
            ...
            border=True,
            help="This is the total amount of funds spent in September.",
        )
    with col3:
        st.metric(
            "September Delta",
            ...
            help="This is the difference in the amount budgeted and the amount spent for September.",
        )
    with col4:
        st.metric(
            "September Variable Cost Budget",
            value=f"£{data_Variable_Budget:,.2f}",
            border=True,
            help="This is the total amount budgeted to be spent for Variable Costs in September.",
        )
    with col5:
        st.metric(
            "September Variable Cost Spent",
            value=f"£{data_Variable_Spent:,.2f}",
            border=True,
            help="This is the total amount spent for Variable Cost inSeptember.",
        )
    with col6:
        st.metric(
            "September Variable Cost Delta",
            value=f"£{data_Variable_Delta:,.2f}",
            border=True,
            delta=f"{data_Variable_Delta_Percent:,.2f}%",
            help="This is the difference in the budgeted amount and Spent amount for Variable Costs in September.",
        )
